...
const {trav, validate} = require('.');

describe('trav', () => {

  describe('identical trav', () => {

    const identicalTrav = (data, schema) => trav(data, schema, ({data}) => data);
    ...
    test('negative', () => {
      ...
      expect(() => identicalTrav('asd', [])).toThrow('asd is not array');
    })
  })
...
describe('validate', () => {
  test('prim', () => {
    ...
    expect(validate(123, 'number')).toEqual(ok);
    expect(validate('123', 'string')).toEqual(ok);
    // expect(validate)
  })
})
